...
读取一张飞书在线表格或知识库 Wiki 中的表格，生成美化后的 Markdown 内容，并用飞书应用机器人发送给一个联系人或群聊。支持 `msg_type: post` + `md` 标签，也支持飞书交互式卡片。
...
工具会先调用知识库节点接口，把 `wiki` 后面的节点 token 转成多维表格 `app_token`，再读取多维表格记录。对多维表格来说，`A1:C16` 会被解释为“取前 3 个字段，最多 15 条数据记录”，第一行作为 Markdown 表头。
...
飞书客户端对 `post` 消息里的 `md` 表格渲染能力可能受客户端版本影响。建议先给自己发一条测试消息；如果表格样式不符合预期，可以改用飞书交互式卡片发送。
